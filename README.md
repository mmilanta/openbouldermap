...
The viewer has a top-left search bar for boulder problems, sectors and areas
worldwide. Its index is a static JSON file (`tiles/climbing-search.json`,
generated by `scripts/extract-search-index.py` during the tile build). It is
fetched lazily on the first interaction with the box, so people who only browse
never download it. Results rank exact, prefix, word-start then substring
