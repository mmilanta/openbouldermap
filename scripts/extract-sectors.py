#!/usr/bin/env python3
"""Extract bouldering sector points from OSM site relations.

Bouldering sectors are often mapped as `type=site` relations with
`climbing=area` + `climbing:boulder=yes` + `site=climbing`, whose members are
the route/boulder nodes that belong to the sector (e.g. "Paese Sector" in
Chironico). Planetiler can't turn a node-membered site relation into a polygon,
so this script reads the osmium-filtered OPL dump and emits one Point feature
per sector, placed at the centroid of its member nodes.
...
def main() -> None:
    ...
    # sector relations: id -> (name, [(type, id), ...])
    sectors: dict[int, tuple[str, list[tuple[str, int]]]] = {}
    ...
    for line in sys.stdin:
        ...
        if otype == "n":
            ...
            # A sector = a site relation grouping bouldering routes/boulders.
            if tags.get("climbing:boulder") == "yes" and tags.get("name"):
                sectors[oid] = (tags["name"], parse_members(member_str))

    # Resolve each sector's members (nodes, ways, nested relations) to node coords.
    features = []
    for rel_id, (name, members) in sorted(sectors.items()):
        ...
        def collect(refs: list[tuple[str, int]], depth: int = 0) -> None:
            ...
            for mtype, mid in refs:
                if mtype == "n":
                    ...
                    if mid in sectors:
                        collect(sectors[mid][1], depth + 1)

        collect(members)
        if not coords:
            print(f"warning: no coordinates for sector {rel_id} ({name})", file=sys.stderr)
            continue
        ...
        features.append(
            {
                ...
                "properties": {"name": name, "osm_id": rel_id, "osm_type": "relation"},
            }
        )
    ...
    print(f"wrote {len(features)} sector points to {OUT}")
